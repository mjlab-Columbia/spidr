...
optional_config_keys = ["cutadapt_oligos"]
...
rule trim_sequencing_adapters:
    ...
    output:
        ...
        path.join(
            ...
            "{experiment}_R1.part_{splitid}.fastq.gz_trimming_report.txt",
        ),
        ...
        path.join(
            ...
            "workup",
            "trim_sequencing_adapters",
            "{experiment}_R2.part_{splitid}.fastq.gz_trimming_report.txt",
        ),
    log:
...
rule identify_barcodes:
    ...
    output:
        r1_barcoded=path.join(
            out_dir, "workup", "identify_barcodes", "{experiment}_R1.part_{splitid}.barcoded.fastq.gz"
        ),
        r2_barcoded=path.join(
            out_dir, "workup", "identify_barcodes", "{experiment}_R2.part_{splitid}.barcoded.fastq.gz"
        ),
    params:
...
rule split_reads_read1:
    ...
    output:
        ...
        short=path.join(
            out_dir, "workup", "split_reads_read1", "{experiment}_R1.part_{splitid}.barcoded_short.fastq.gz"
        ),
    log:
...
rule split_reads_read2:
    ...
    output:
        ...
        short=path.join(
            out_dir, "workup", "split_reads_read2", "{experiment}_R2.part_{splitid}.barcoded_short.fastq.gz"
        ),
    log:
...
rule trim_bead_oligo_reads:
    ...
    output:
        fastq=path.join(
            out_dir, "workup", "trim_bead_oligo_reads", "{experiment}_R1.part_{splitid}.barcoded_bpm.RDtrim.fastq.gz"
        ),
        qc=path.join(
            out_dir, "workup", "trim_bead_oligo_reads", "{experiment}_R1.part_{splitid}.barcoded_bpm.RDtrim.qc.txt"
        ),
    params:
...
rule align_bowtie2:
    ...
    output:
        ...
        unmapped=path.join(
            out_dir, "workup", "align_bowtie2", "{experiment}.part_{splitid}.bowtie2.sorted.unmapped.bam"
        ),
...
rule align_star:
    input:
        ...
        r2=path.join(out_dir, "workup", "convert_bam_to_fastq", "{experiment}.part_{splitid}.bowtie2.unmapped_R2.fq.gz"),
    output:
        sorted_by_coord=path.join(
            out_dir, "workup", "align_star", "{experiment}.part_{splitid}.Aligned.sortedByCoord.out.bam"
        ),
        index=path.join(
...
rule add_chromosome_info_bowtie2:
    ...
    output:
        bt2=path.join(
            ...
            "{experiment}.part_{splitid}.bowtie2.sorted.mapped.chr.bam",
        ),
    params:
...
rule add_chromosome_info_star:
    ...
    output:
        star=path.join(
            ...
            "{experiment}.part_{splitid}.Aligned.out.sortedByCoord.chr.bam",
        ),
    params:
        assembly=config["assembly"],
    log:
...
rule generate_splitbam_statistics:
    input:
        expand(
            [path.join(out_dir, "workup", "splitbams_all_conditions", "{experiment}.done")], experiment=ALL_EXPERIMENTS
        ),
        expand(
            [path.join(out_dir, "workup", "splitbams_by_condition", "{experiment}.done")], experiment=ALL_EXPERIMENTS
        ),
    output:
